...
from common import DynamoDBClient, get_logger, DatabaseError, TransactionType
    ...
    try:
        ...
        for child in children:
            try:
                ...
                if interest_rate == 0:
                    logger.debug(f"Skipping {user_id} ({name}): interest rate is 0")
                    skipped += 1
                    continue

                if current_balance == 0:
                    logger.debug(f"Skipping {user_id} ({name}): balance is 0")
                    skipped += 1
                    continue
